-------------------------------------------------------------------------------
-- ##String API
-- The [string API](http://www.lua.org/manual/5.1/manual.html#5.4) provides 
-- generic functions for `string` manipulation, such as finding and extracting
-- substrings, and pattern matching.
-- ###Notes:  
-- >
-- > 1. When indexing a `string` in Lua, the first character is at position `1`
-- (not at `0`, as in C). 
-- Indices are allowed to be negative and are interpreted as indexing backwards,
-- from the end of the `string`. Thus, the last character is at position -1, and
-- so on.
--
-- > 2. The string library provides all its functions inside the `table` string.
...
--
-- > 3. The string library assumes one-byte character encodings.
-- @module string
...
-- ###Notes:  
-- >
-- > 1. The numerical codes are not necessarily portable across platforms.
...
-- @return #Untyped the internal numerical codes of the characters `s[i]`, `s[i+1]`,..., `s[j]`
...
-- @return #string a `string` with length equal to the number of arguments, in 
...
-- ###Notes:  
-- >
-- > 1. If `plain` is given, then `init` must be given as well.
-- > 2. If the pattern has captures, then in a successful match the captured values
-- are also returned, after the two indices.
-- ###Warnings:  
-- >
-- > 1. Calling `string.find` on a `string` returned by `string.sub` will return the
-- index of the character in the original `string`, not the substring you are
-- referencing. 
...
-- ###Warnings:  
-- >
-- > 1. Using any quantifier other than + will cause it to hang if that
-- quantifier applies to the whole pattern and that pattern does not match
-- the whole `string`. 
-- @function [parent=#string] gmatch
-- @param #string s `string` to handle.
-- @param #string pattern pattern to search.
-- @return #func Returns an iterator `function` that, each time it is called, returns the next captures from pattern over `string` `s`.
...
-- @param #number j end index. (default value is `-1`, which is the same as the `string` length)
