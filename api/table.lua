-------------------------------------------------------------------------------
-- ##Table API
-- The [table API](http://www.lua.org/manual/5.1/manual.html#5.5) provides
-- generic functions for table manipulation.
...
-- @param #number j end index, default value is length of the `table`.
-- @return #string `string` of the concatenated `table`.
...
-- @param #table table `table` to modify.
-- @param #number pos index of deletion. (default value is the length of the `table`)
...
-- ###Notes:  
-- >
-- > 1. Use a `for loop` with `pairs` or `ipairs` instead. 
...
-- ###Notes:  
-- >
-- > 1. `Function` `table.getn` corresponds to the new length operator (#);
